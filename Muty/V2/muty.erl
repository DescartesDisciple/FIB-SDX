...
start(Lock, Sleep, Work) ->
    ...
    spawn('p1@127.0.0.1', fun() -> register(l1, apply(Lock, start, [1, self()])) 
                          end),
    spawn('p2@127.0.0.1', fun() -> register(l2, apply(Lock, start, [2, self()])) 
                          end),
    spawn('p3@127.0.0.1', fun() -> register(l3, apply(Lock, start, [3, self()])) 
                          end),
    spawn('p4@127.0.0.1', fun() -> register(l4, apply(Lock, start, [4, self()])) 
                          end),
    ...
    %timer:sleep(2000),
% Ponemos timer a la espera de saber cómo coño funciona receive
    ...
    spawn('p1@127.0.0.1', fun() -> register(w1, worker:start("John", l1, Sleep, Work)) 
                          end),
    spawn('p2@127.0.0.1', fun() -> register(w2, worker:start("Ringo", l2, Sleep, Work)) 
                          end),
    spawn('p3@127.0.0.1', fun() -> register(w3, worker:start("Paul", l3, Sleep, Work)) 
                          end),
    spawn('p4@127.0.0.1', fun() -> register(w4, worker:start("George", l4, Sleep, Work)) 
                          end),
	ok.
